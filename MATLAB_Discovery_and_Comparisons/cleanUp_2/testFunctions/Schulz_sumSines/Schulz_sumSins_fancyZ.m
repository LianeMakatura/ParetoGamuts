function [fun, deriv, f]= Schulz_sumSins_fancyZ(N_Order, rD, origMin, origMax, z)
...
if ~exist('z', 'var') %% we're in the unconstrained z case, where z is a design var
    disp('sumSins_fancyZ unconstrained z!')
else
    disp('sumSins_fancyZ fixed z')
end
...
if exist('z', 'var')
    ...
    fun = @(x, z)Schulz_sumSinsHelper_fancyZ(randA, randB, totalMin - 0.1, totalMax + 0.1, x, z);
    f(A) = Schulz_sumSinsHelper_fancyZ(randA, randB, totalMin - 0.1, totalMax + 0.1, A.', z);
else % unconstrained z
    A = sym('x', [1 rD]);

    fun = @(x)Schulz_sumSinsHelper_fancyZ(randA, randB, totalMin - 0.1, totalMax + 0.1, x);
    f(A) = Schulz_sumSinsHelper_fancyZ(randA, randB, totalMin - 0.1, totalMax + 0.1, A.');
end
