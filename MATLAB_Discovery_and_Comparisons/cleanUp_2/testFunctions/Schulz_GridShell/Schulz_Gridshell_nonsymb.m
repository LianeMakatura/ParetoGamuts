classdef Schulz_Gridshell_nonsymb
    ...
    methods
        function obj = Schulz_Gridshell_nonsymb(basewd, numX, numY, perfMetricIDs, fixed_context)
            %PROBLEMTEMPLATE - constructor for the class. Named same as
            % class.
            %   Detailed explanation goes here
            obj.problemName = "Schulz_Gridshell_nonsymb"; %shouldn't have any spaces or special chars except underscore
...
%             appNames = ["House Orientation"];
            obj.zSymb = sym('z', [1 1]); % need to have at least 1 symbolic var for the framework to be happy. Just don't use it if there shouldn't be one.
...
%                 [fun, ffunc, hess] = obj.setupFunction(fhandle, fname, createFiles, optimizeMatlabFuncs);
                [fun, ffunc, deriv, hess] = obj.setupFunction(fhandle, fname, createFiles, optimizeMatlabFuncs);
...
%             rderiv = str2func(deriv_name);
            % check rderiv is a cell
            rderiv = @(x, z)obj.createJacobian(rderiv_sub, x, z);
        end
        ...
        function [fun, f, deriv, hess] = setupFunction(obj, fHandle, fname, createFile, optimizeMatlabFuncs)
            if createFile
            ...
            d = jacobian(f, [obj.xSymb, obj.zSymb]); 
            deriv_name = fname + "_deriv";
            if createFile
                matlabFunction(d, 'File', char(obj.wd +"generated/"+ deriv_name), 'vars',{obj.xSymb.', obj.zSymb.'}, 'Optimize', optimizeMatlabFuncs);
            end
            deriv = str2func(deriv_name);
            hess = @(x, z)fast_hess(f_func, deriv, x, z);
