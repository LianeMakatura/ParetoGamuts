classdef LBracket_stress
    ...
    methods
        function obj = LBracket_stress(basewd, threeD_bool, constrained_bool, zValFixed)
            ...
            obj.problemName = "Lbracket";
            desNames = ["Arm length", "Arm width", "Arm thickness"];
            appNames = ["Bend Angle"];
            perfNames = ["Mass", "Stress"];
        ...
        function [rfun, rf, rfuncsCell, rderiv] = createProblem(obj, createFiles)
            ...
            if createFiles    
                % only write the first symbolic one, because the second
                % metric was hardcoded independently (no symbolic rep)
                f1 = obj.mass();
