classdef Schulz_Lamp
    ...
    methods
        function obj = Schulz_Lamp(basewd, perfMetricIDs, fixed_context)
            %PROBLEMTEMPLATE - constructor for the class. Named same as
            % class.
            %   Detailed explanation goes here
            obj.problemName = "Schulz_Lamp"; %shouldn't have any spaces or special chars except underscore
            ...
            obj.beamRanges = struct('base', struct('x', [0,1], 'y', [0,1], 'z', [1,2]),...
...
%             obj.zSymb = sym('z', [1 obj.rA]);
            obj.zSymb = fixed_context;
            
            % Create aliases for the variables so they're meaningful, 
            % and scale as necessary
            scale = obj.bMax+obj.aMax;
            ...
            % ========== PERFORMANCE METRICS ============
            obj.rd = length(perfMetricIDs); %need to modify code for 3, start with 2
        ...
        function f2 = performanceMetric2(obj)
            ...
            maxMass  = norm([1,1,2])*(obj.hMax*3 + obj.aMax*3 + obj.bMax)*(2*obj.wMax);
        ...
        function [rfun, rf1, rf2, rf3, rderiv1, rderiv2, rderiv3] = createProblem(obj, createFiles)
        ...
        function [fun, f, deriv] = setupFunction(obj, fHandle, fname, createFile)
            if createFile
                matlabFunction(fHandle, 'File', char(obj.wd + fname), 'vars',{obj.xSymb.'},... 
